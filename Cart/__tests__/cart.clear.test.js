...
describe('Delete /api/cart/items', () => {
    ...
    it('returns 200 and clear all items from the cart', async () => {
        const res = await request(app).delete('/api/cart/items').set(`Authorization`, `Bearer ${token}`)

        expect(res.statusCode).toBe(200);
        expect(res.body.cart.items).toHaveLength(0);
    });
    it('returns 401 when no auth is provided', async () => {
        const res = await request(app).delete('/api/cart/items')
    ...
    it('returns 401 when invalid token is provided', async () => {
        ...
        const res = await request(app).delete('/api/cart/items').set(`Authorization`, `Bearer ${faketoken}`)
    ...
    it('returns 403 when role is not user', async () => {
        ...
        const res = await request(app).delete('/api/cart/items').set(`Authorization`, `Bearer ${admintoken}`)

        expect(res.statusCode).toBe(403);
        expect(res.body.message).toBe('Forbidden: insufficient permissions');
    });
    it('returns 404 if cart is not exists',async()=>{
